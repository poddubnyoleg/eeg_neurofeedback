...
class OnlineFilter:
    def __init__(self, fs, notch_f0, notch_q, low_cut, high_cut, order):
        ...
        self.notch_b, self.notch_a = iirnotch(notch_f0 / (fs / 2.0), notch_q)
        self.butter_b, self.butter_a = butter(
            order, [low_cut / (0.5 * fs), high_cut / (0.5 * fs)], btype="band"
        )

    def filter(self, data):

        if type(self.zi_butter) != np.ndarray:
            y, self.zi_notch = lfilter(
                self.notch_b,
                self.notch_a,
                data,
                zi=lfilter_zi(self.notch_b, self.notch_a) * data[0],
            )
            y, self.zi_butter = lfilter(
                self.butter_b,
                self.butter_a,
                y,
            ...
            y, self.zi_notch = lfilter(
                self.notch_b, self.notch_a, data, zi=self.zi_notch
            )
            y, self.zi_butter = lfilter(
                self.butter_b, self.butter_a, y, zi=self.zi_butter
            )
...
def spectral_features(x):
    ...
    return pd.Series(
        20 * np.log10(np.abs(np.fft.rfft(x * np.hanning(len(x)), n=256 * 2))[:80])
    )
